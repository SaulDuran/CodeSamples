...
if(isset($_POST['submit'])){
	...
	if($postSlug == "plataformas-de-innovacion-estrategica"){$postSlug = "que";}
	//POST CONTENT
	...
	$childPages = get_pages( array( 'child_of' => $id, 'sort_column' => 'menu_order','post_type' => strtolower($postSlug) ) );
	if ( $childPages ) {
        foreach ( $childPages as $childPage ) {
			$output_images_pages[] = wp_get_attachment_url( get_post_thumbnail_id($childPage->ID) );
			$pages_child[$childPage->post_title] = apply_filters ("the_content", $childPage->post_content);
        }
     }
